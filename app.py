...
class AnalysisConfig:
    """分析参数配置"""
    # 姿态检测
    model_complexity: int = 2           # 0=轻量, 1=标准, 2=精准
    min_detection_conf: float = 0.5
    min_tracking_conf: float = 0.5

    # 时序分析
    history_length: int = 45            # 用于动作分类的帧历史
    hit_cooldown: int = 15              # 击球冷却帧数（避免重复检测）

    # 分类阈值
    speed_threshold: float = 30.0       # 手腕速度阈值（像素/帧）
    hit_confidence: float = 0.65        # 动作识别最低置信度
...
class PoseEstimator:
    ...
    def _download_model(self) -> str:
        ...
        if not os.path.exists(model_path):
            ...
            st.info("⏳ 首次运行正在下载 AI 模型（约 12MB），请稍候...")
...
def extract_features(pose_data):
    """从姿态数据中提取角度特征，返回 (特征字典, 手腕像素坐标 (x,y))"""
    ...
    # ✅ 修正点1：返回手腕坐标为 (x, y) 元组，而不是字典
    wrist_pos = (pts[f'{s}_wrist']['x'], pts[f'{s}_wrist']['y'])
    return features, wrist_pos

# ==================== 动作分类器（规则引擎 + 预留 Transformer 接口）====================
class TransformerClassifier:
    """真实的 Transformer 分类器接口（替换为训练好的模型）"""
    def __init__(self):
        # 这里可以加载 PyTorch/TensorFlow 模型
        self.model = None
        self.label_map = {0: 'smash', 1: 'forehand_clear', 2: 'drop_shot', 3: 'net_kill'}

    def predict(self, sequence_features):
        """
        sequence_features: numpy array shape (seq_len, feature_dim)
        返回: (动作标签, 置信度)
        """
        # 占位：现在用规则引擎替代，实际可替换为模型推理
        # return self.label_map[model.predict(sequence_features)], 0.9
        return None, 0.0  # 返回 None 则触发规则引擎
...
class ActionDetector:
    ...
    def __init__(self, config: AnalysisConfig):
        ...
        self.frame_idx = 0
        self.transformer = TransformerClassifier()  # 可扩展

    def update(self, features, wrist_pos):
        """输入新一帧特征，返回 (是否为击球帧, 动作类型, 置信度)"""
        self.frame_idx += 1
        self.feature_history.append(features)
        self.wrist_history.append(wrist_pos)  # wrist_pos 是 (x,y) 元组
    ...
    def evaluate(self, angles, action_type):
        ...
        for joint, ideal_val in ideal.items():
            if joint in angles:
                ...
                score = max(0, 100 - diff * 0.8)  # 简单线性评分
...
def analyze_video(video_path, config, progress_callback):
    ...
    frame_idx = 0
    last_vis = None  # ✅ 修正点2：初始化 last_vis 以防无姿态帧
...
# ==================== Streamlit 界面 ====================
st.title("🏸 羽毛球智能动作分析系统 (MediaPipe 新版)")
